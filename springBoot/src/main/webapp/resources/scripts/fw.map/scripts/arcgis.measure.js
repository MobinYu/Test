...
DCI.Measure = {
    ...
    */
    Init: function (map) {
        ...
        this.lineSymbol = new esri.symbol.SimpleLineSymbol(esri.symbol.SimpleLineSymbol.STYLE_SOLID, new dojo.Color([255, 0, 0]), 2); //[255, 99, 71]
        this.fillSymbol = new esri.symbol.SimpleFillSymbol(esri.symbol.SimpleFillSymbol.STYLE_SOLID, new esri.symbol.SimpleLineSymbol(esri.symbol.SimpleLineSymbol.STYLE_SOLID, new dojo.Color([255, 0, 0]), 2), new dojo.Color([255, 255, 255, 0.5])); //255, 160, 122
    ...
    measureDistance: function () {
        ...
        this.setMapCursor("url('styles/images/ruler.cur'),auto");
    ...
    deactivate: function () {
        DCI.Measure.meatureTool.deactivate();
...
DCIMeature = DObject({
    ...
    construct: function (map) {
        ...
        this._graphicsLayer = new esri.layers.GraphicsLayer({ id: "DrawLayer" });
    },
    ...
    _onClickHandler: function (evt) {
        ...
        if (this._meatureType == DCIMeatureType.DISTANCE) {
            ...
            var picSymbol = new esri.symbol.PictureMarkerSymbol('styles/images/symbol/round.png', 12, 12);
            var graphicsPic = new esri.Graphic(pt, picSymbol);
            this._graphicsLayer.add(graphicsPic);

            this._DGraphics[this._DCount - 1].push(graphicsText);
            this._DGraphics[this._DCount - 1].push(graphicsPic);
    ...
    terminate: function () {
        if (this._isEnd == false && this._DClickNum!=-1) {//by MHQ 只有开始测量点击后才把数据删除
            if (this._meatureType == DCIMeatureType.DISTANCE) {
    ...
    _checkDistance: function () {
        ...
        if (this._DCount > 0) {
            ...
            var url = "styles/images/symbol/delete.png";
            var delGraphic = new esri.Graphic(pt, new esri.symbol.PictureMarkerSymbol(url, 16, 16).setOffset(12, -12));
