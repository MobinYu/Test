require([
  ...
  PopupTemplate, GraphicsLayer, Font
    ...
    return declare("esri.ClusterLayer", [GraphicsLayer], {
        constructor: function (options) {
            ...
            this._singleSym = options.singleSymbol || new SMS("circle", 10, new esri.symbol.SimpleLineSymbol(esri.symbol.SimpleLineSymbol.STYLE_SOLID, new Color([255, 255, 0]), 1), new Color([255, 255, 0, 0.3]));
            this._singleTemplate = options.singleTemplate || new PopupTemplate({ "title": "", "description": "{*}" });
            this._maxSingles = options.maxSingles || 1000;

            this._webmap = options.hasOwnProperty("webmap") ? options.webmap : false;

            this._sr = options.spatialReference || new SpatialReference({ "wkid": 4326 }); //by MHQ 102100

            this._zoomEnd = null;
            this._onClickEvent = options.onClickEvent;
            this._rendererSingleFuc = options.rendererSingleFuc;
            this._rendererSingleField = options.rendererSingleField;
            this._showMinScale = options.showMinScale;
        },
        ...
        _setMap: function (map, surface) {
            ...
            this._zoomEnd = connect.connect(map, "onZoomEnd", this, function () {
                ...
                //this._clusterGraphics();
                //by MHQ
                if (this._map.getScale() > this._showMinScale) {//this._showMinScale != 0 &&
                    map.infoWindow.hide();
                    this._clusterGraphics();
                }
            });
        ...
        onClick: function (e) {
            ...
            if (singles.length > this._maxSingles) {
                ...
                alert("很抱歉, 当前超过 " + this._maxSingles + " 个点。请放大地图再看！");
                return;
            }
            else {
                // stop the click from bubbling to the map
                e.stopPropagation();
                this._map.graphics.clear(); //移除3.14默认的高亮标注
        ...
        new Graphic(
          ...
          c.attributes
            ...
            //by MHQ
            if (c.attributes.clusterCount == 1) {
                var symbol = new esri.symbol.PictureMarkerSymbol(c.attributes.symbolUrl, 32, 32).setOffset(0, 0);
                var graphic = new esri.Graphic(point, symbol, c.attributes);
                this.add(graphic);
                return;
            }
        ...
        .setOffset(0, this._clusterLabelOffset).setFont(font); ;
            this.add(
        new Graphic(
          point,
          label,
        ...
        _addSingles: function (singles, geometry) {
            // add single graphics to the map
            var currentGeo = undefined;
            arrayUtils.forEach(singles, function (p) {
                if (this._rendererSingleField && $.isFunction(this._rendererSingleFuc)) {
                    this._singleSym = this._rendererSingleFuc(p.attributes[this._rendererSingleField]);
                };
                var g = new Graphic(
          ...
          this._singleTemplate
                ...
                if (p.x == geometry.x && p.y == geometry.y) {
                    currentGeo = g;
                }
            }, this);
