$.ToolsControl = {
    ...
    , _DefaultShowToolNameArray: [
                "MeasureClearAll", "MeasureDistance", "MeasureArea", "MeasurePoint", "AddPoint"
            ...
            , _DefaultToolTypes: [
                {
                    ...
                    , Tools: [
                        ...
                        , {
                            ...
                            , ClassName: "PrintMap"
                            , OnClick: function (e, data, OnComplete) {
                                //API.ArcGISAPI.createPrint();
                                divArcGISPrintButtonControlJQ.toggle();
                            }
                            , OnComplete: function (data) {
                    ...
                    , Tools: [
                        ...
                        , {
                            ...
                            , OnComplete: function (e, data) {
                                //alert(data.ArcGISMap);
                                var Settings = {
                                    geometry: e.geometry//多边形
                                    , layerName: "testErase"
                                    , outPutName: "output"//输出参数名称
                                    , eraseMaskName: "maskErase"
                                    , gpLayerUrl: EraseModelToolUrl
                                };
                                ArcGISAPI.eraseLayerGPServer(Settings);
                            }
                        }
                        , {
                    ...
                    , Tools: [
                        {
                            ...
                            , OnClick: function (e, data, OnComplete) {
                                data.ArcGISWindow.ArcGISAPI.clearMapDrawGraphics();
                                data.ArcGISWindow.ArcGISAPI.businessLayerRemove({ keyword: "geoQuery" });
                        ...
                        , {
                            ...
                            , OnComplete: function (e, data) {
                                var graphicList = []; var i = 0;
                                //断面
                                for (i = 0, len = sectionDataList.length; i < len; i++) {
                                    if (e.geometry.contains(new esri.geometry.Point(sectionDataList[i].posX, sectionDataList[i].posY))) {
                                        graphicList.push(sectionDataList[i]);
                                    }
                                }
                                var ptSymbol = GetDefaultSymbolByName(SymbolNames.Point_Picture, { image: "styles/images/symbol/waterSection.png" });
                                $.sectionControl.addSectionPt({ LayerName: "geoQuery_sectionLayer", Symbol: ptSymbol, GraphicList: graphicList });

                                //饮用水源地
                                graphicList = []; i = 0;
                                for (i = 0, len = drinkwaterDataList.length; i < len; i++) {
                                    if (e.geometry.contains(new esri.geometry.Point(drinkwaterDataList[i].posX, drinkwaterDataList[i].posY))) {
                                        graphicList.push(drinkwaterDataList[i]);
                                    }
                                }
                                ptSymbol = GetDefaultSymbolByName(SymbolNames.Point_Picture, { image: "styles/images/symbol/drinkwater.png" });
                                $.drinkwaterControl.addDrinkwaterPt({ LayerName: "geoQuery_drinkwaterLayer", Symbol: ptSymbol, GraphicList: graphicList });

                                //15条河流水质站
                                graphicList = []; i = 0;
                                for (i = 0, len = lakeStationDataList.length; i < len; i++) {
                                    if (e.geometry.contains(new esri.geometry.Point(lakeStationDataList[i].posX, lakeStationDataList[i].posY))) {
                                        graphicList.push(lakeStationDataList[i]);
                                    }
                                }
                                ptSymbol = GetDefaultSymbolByName(SymbolNames.Point_Picture, { image: "styles/images/symbol/lakeStation.png" });
                                $.lakeControl.addLakeStationPt({ LayerName: "geoQuery_lakeStationLayer", Symbol: ptSymbol, GraphicList: graphicList });


                                var legendSettings = {
                                    ArcGISWindow: API.ArcGISAPI.ArcGISWindow,
                                    ArcGISMap: API.ArcGISAPI.ArcGISMap,
                                    //LayerArray: [{ LayerName: "RiverSiteLayer", AttrName: "type"}],
                                    legend: [
                                        { name: "重点断面", image: "styles/images/symbol/waterSection.png" },
                                        { name: "饮用水源地", image: "styles/images/symbol/drinkwater.png" },
                                        { name: "水质站", image: "styles/images/symbol/lakeStation.png" }
                                    ]
                                };
                                divArcGISLegendJQ.LegendControl(legendSettings);
                            }
                        }
                        ]
                 }
    ...
    ToolsControl: function (Properties) {
        ...
        this.each(function () {
            ...
            if (!fw.fwObject.FWObjectHelper.hasValue(ControlData)) {
                ...
                Html += "        <div class=\"divToolsImage\"></div>";
                Html += "    </div>";
