    ...
    if (typeof dojox.gfx.svg.Text.prototype.setShape == 'function') {
        dojox.gfx.svg.Text.prototype.setShape = function (p) {
            ...
            if (s.text) {
                if (s.text.indexOf("|") > -1) {
                    isCenter = true;
                    s.text = s.text.substring(0, s.text.indexOf("|"));
                };
                ...
                for (var i = 0, n = texts.length; i < n; i++) {
                    ...
                    if (isCenter && i > 0) {
                        var p = texts[i].length - texts[0].length;
                        //tspan.setAttribute("x", s.x - lineHeight * 2);
                        tspan.setAttribute("x", s.x - p / 4 * lineHeight);
                    } else {
