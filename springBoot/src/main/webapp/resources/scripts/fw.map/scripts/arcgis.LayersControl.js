                    ...
                    if (!fw.fwObject.FWObjectHelper.hasValue(Layer.OnClick)) {
                        Layer.OnClick = function (e, data, OnComplete) {
                            ...
                            if ($(e.currentTarget).is(":checked")) {
                                ...
                                } else if (fw.fwObject.FWObjectHelper.hasValue(data.Layer.callback)) {
                                    try {
                                        data.Layer.callback(data.Layer.LayerName, data.Layer.LayerTitle);
                                    } catch (ex) { };
                                } else {
                                    try {
                                ...
                                } else if (fw.fwObject.FWObjectHelper.hasValue(data.Layer.outCallback)) {
                                    try {
                                        data.Layer.outCallback(data.Layer.LayerName, data.Layer.LayerTitle);
                                    } catch (ex) { };
                                } else {
                                    try {
...
$.fn.extend({
    LayersControl: function (Properties) {
        ...
        this.each(function () {
            ...
            if (!fw.fwObject.FWObjectHelper.hasValue(ControlData)) {
                ...
                Html += "        <div class=\"divLayersImage\"></div>";
                Html += "    </div>";
    ...
    , RiskLayersControl: function (Properties) {
        var Settings = {
            ArcGISWindow: null
                    , ArcGISMap: null
                    , LayerTypes: null
        ...
        this.each(function () {
            ...
            LayersControlJQ.data("ControlData", ControlData).empty();
            LayersControlJQ.css("position", "absolute");
            LayersControlJQ.css({ "top": Settings.Top + "px", "left": Settings.Left + "px" });
            ...
            Html += "<div class=\"jQE_Container_Absolute\">";
            Html += "    <div class=\"divLayers\">";
            Html += "        <div class=\"divLayersImage\">" + Settings.title + "</div>";
            Html += "    </div>";
            Html += "    <div class=\"divLayersMenu divArcGISControl\">";
            Html += "        <table class=\"tableLayersMenu\">";
            Html += "            <tr>";
            Html += "                <td style=\"vertical-align: top;display:none;\">";
            Html += "                    <div class=\"divLayersType\">";
            Html += "                        <ul class=\"ulLayersType\"></ul>";
            Html += "                    </div>";
            Html += "                </td>";
            ...
            ControlData.ControlJQs.LayersImageJQ.click();
        });
        return this;
    }
    ...
    //弹出框(加载页面)
         , PopUpFrameBoxShow: function (Properties) {
           ...
           , PopUpFrameBoxHide: function (Properties) {
